...
class ClipCommands(commands.Cog):
    ...
    async def cmd_clip(self, ctx: commands.Context):
        ...
        clip = await self.bot.clipper.create_clip(
            self.bot.cfg.broadcaster_id, title=f"Clipped by {ctx.author.name}"
        )
        if clip:
            self.bot.highlight_mgr.record_clip(clip)
            await ctx.send(f"✂️ Clipped by {ctx.author.name}! {clip['edit_url']}")
            await self.bot.alerter.send_clip_alert(clip["edit_url"], ctx.author.name)
        else:
